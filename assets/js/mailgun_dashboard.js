...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_0__css_mailgun_dashboard_scss__ = __webpack_require__(1);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_0__css_mailgun_dashboard_scss___default = __webpack_require__.n(__WEBPACK_IMPORTED_MODULE_0__css_mailgun_dashboard_scss__);
...
var MailgunDashboardDashboard = function ($) {
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                if (response.success) {
                    ...
                    logTable.DataTable({
                        'data': data,
                        'columns': [{ defaultContent: '', width: '1%' }, { data: 'timestamp', width: '15%' }, {
                            data: 'event',
                            width: '15%',
    ...
    self.getMailgunStats = function (dateRangeStart, dateRangeEnd) {
        var data = {
            ...
            type: 'stats',
            resolution: 'hour',
            events: ['delivered', 'failed', 'stored', 'accepted', 'opened', 'clicked', 'unsubscribed', 'complained'],
            dateRangeStart: dateRangeStart.unix(),
            dateRangeEnd: dateRangeEnd.unix()
        };
            ...
            success: function (response) {
                ...
                if (response.success) {
                    ...
                    $.each(JSON.parse(response.data).items, function (index, value) {
                        var newRow = '<tr>' + '<td class="status ' + value.hap + '" title="' + mailgunDashboardDashboardTexts.eventStatus[value.hap] + '"></td>' + '<td>' + value.created_at + '</td>' + '<td>' + value.message + '</td>' + '</tr>';
                    ...
                    logTable.DataTable({
                        ...
                        bAutoWidth: false,
                        aoColumns: [{ sWidth: '1%' }, { sWidth: '15%' }, { sWidth: '73%' }],
                        'language': {
                            'decimal': mailgunDashboardDashboardTexts.decimal,
                            'emptyTable': mailgunDashboardDashboardTexts.emptyTable,
                            'info': mailgunDashboardDashboardTexts.info,
    ...
    $(document).on('click', '.js-mgd-refresh-dashboard-button', function () {
        var range, statsChartContainer, dateRangeContainer, eventsTableContainer;
    ...
    self.dateRangeCallback = function (dateRangeStart, dateRangeEnd) {
        ...
        var eventsTableContainer, statsChartContainer;
...
/***/ })
/******/ ]);
//# sourceMappingURL=mailgun_dashboard.js.map